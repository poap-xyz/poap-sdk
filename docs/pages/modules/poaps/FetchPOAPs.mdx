...
```typescript
import { POAPWithStats } from '@poap-xyz/poap-sdk';

const poapId = 999;

const poap: POAPWithStats = await client.get(poapId);
```
...
const paginatedPOAPs: PaginatedResult<POAPWithStats> = await client.list({
  offset: 0,
  limit: 10,
...
const collectorPOAPs: PaginatedResult<POAPWithStats> = await client.list({
  offset: 0,
  limit: 10,
  sortField: PoapsSortFields.MintedOn,
  sortDir: Order.ASC,
...
const dropPOAPs: PaginatedResult<POAPWithStats> = await client.list({
  offset: 0,
  limit: 10,
  sortField: PoapsSortFields.MintedOn,
  sortDir: Order.ASC,
